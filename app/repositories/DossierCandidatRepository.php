...
class RechercheLocalisationService
{
	...
	/* COLONNES                      */
	/*-------------------------------*/
	public const COLONNE = ['Etablissement', 'Commune', 'Code Postal', 'Departement', 'Pays'];
	...
	private function appelerPhoton($commune, $codePostal, $pays)
	{
		...
		$adressePropre = implode(' ', $elements);
		$recherche     = urlencode($adressePropre);
		...
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
		curl_setopt($ch, CURLOPT_USERAGENT, 'ApplicationEtudiante_RechercheLocale');
		...
		return false; // CORRECTION : était une récursion infinie
	}
	...
	public function parcoursTableau($csvBrut)
	{
		...
		$colonnesCibles = ['Etablissement', 'Commune', 'Code Postal', 'Departement', 'Pays', 'longitude', 'latitude', 'result_status'];
		...
		while (($ligne = fgetcsv($fluxMemoire, 0, $delimiteur, '"', "\\")) !== false)
		{
			if (count($entetes) === count($ligne))
			{
				...
				{
					$coordonnees = $this->appelerPhoton($attTableau['Commune'], $attTableau['Code Postal'], $attTableau['Pays']);

					usleep(1000000); // CORRECTION : le délai s'applique toujours après un appel Photon
